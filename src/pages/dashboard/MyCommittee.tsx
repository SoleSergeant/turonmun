...
function MyCommittee() {
  ...
  const calculateProgress = () => {
    ...
    const s = (positionPaperRecord as any)?.status;
    if (s && s !== 'draft') progress += 50; // submitted or graded
    return progress;
  };

  const progressPercentage = calculateProgress();

  // The chair grades with 'approved' / 'needs_revision' / 'reviewed' — treat all
  // of those as "reviewed" on the delegate side so the badge & progress are right.
  const rawPaperStatus = (positionPaperRecord as any)?.status as string | undefined;
  const normalizedPaperStatus: 'draft' | 'submitted' | 'reviewed' =
    ['reviewed', 'approved', 'needs_revision'].includes(rawPaperStatus || '')
      ? 'reviewed'
      : rawPaperStatus === 'submitted'
        ? 'submitted'
        : 'draft';

  const positionPaperStatus = {
    submitted: normalizedPaperStatus !== 'draft',
    deadline: 'TBA',
    feedback: null,
    status: normalizedPaperStatus,
  };

  const [uploading, setUploading] = useState(false);

  const handlePaperUpload = async (file: File | undefined) => {
    if (!file || !application || !committee) return;
    ...
    try {
      setUploading(true);
      const path = `position_papers/${(application as any).id}_${Date.now()}.${ext}`;
      ...
      const { data, error } = await (supabase.from('position_papers') as any)
        .upsert(payload).select('*').single();
      if (error) throw error;
      if (data) setPositionPaperRecord(data as PositionPaperRow);
      toast({ title: 'Position paper submitted', description: file.name });
    } catch (err: any) {
        ...
        {activeTab === 'position-paper' && (
          <div className="space-y-6">
            ...
            <div className="glass-card p-8 border-white/10 relative overflow-hidden">
              ...
              <div className="space-y-6">
                {!application?.assigned_committee_id ? (
                  <div className="glass-panel p-8 rounded-2xl border-dashed border-white/10 text-center">
                    <p className="text-white/40 text-sm">You'll be able to upload your position paper once you're allocated to a committee.</p>
                  </div>
                ) : positionPaperRecord?.file_url ? (
                  // ── Already submitted: show file + replace ──
                  <div className="glass-panel p-6 rounded-2xl border-white/10 bg-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-xl bg-emerald-400/15 border border-emerald-400/30 flex items-center justify-center">
                        <FileText className="h-6 w-6 text-emerald-400" />
                      </div>
                      <div>
                        <p className="text-white font-semibold text-sm">Position paper submitted</p>
                        <a
                          href={positionPaperRecord.file_url}
                          target="_blank"
                          rel="noreferrer"
                    ...
                    <label className="flex items-center justify-center gap-2 glass-panel px-5 py-3 text-white/70 hover:text-white hover:bg-white/10 transition-all rounded-xl border-white/10 text-xs font-bold uppercase tracking-widest cursor-pointer">
                  ...
                  // ── No submission yet: dropzone ──
                  <label className="block cursor-pointer">
                    <div className="border-2 border-dashed border-white/15 rounded-2xl p-10 text-center hover:border-gold-400/40 hover:bg-white/5 transition-all">
                      <Upload className="h-10 w-10 text-white/30 mx-auto mb-3" />
                      <p className="text-white font-semibold mb-1">
                        {uploading ? 'Uploading…' : 'Upload your position paper'}
                      </p>
                      <p className="text-white/40 text-xs">PDF or Word document (.pdf, .doc, .docx)</p>
                    </div>
                    <input
                      type="file"
                      accept={ACCEPT}
        ...
        {activeTab === 'schedule' && (
          <div className="glass-card p-8 border-white/10 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 relative z-10">
              ...
              <motion.button
                className="flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white/70 hover:text-white hover:bg-white/10 transition-all text-xs font-bold uppercase tracking-widest disabled:opacity-40"
                ...
                onClick={() => {
                  const lines = [
                    'BEGIN:VCALENDAR',
                    'VERSION:2.0',
                    'PRODID:-//TuronMUN//Committee Schedule//EN',
                  ];
                  scheduleEvents.forEach((ev) => {
                    const dateStr = ev.event_date?.replace(/-/g, '') || '';
                    const startTime = ev.start_time?.replace(/:/g, '').slice(0, 4) || '0900';
                    const endTime   = ev.end_time?.replace(/:/g, '').slice(0, 4)   || '1000';
                    lines.push(
                      'BEGIN:VEVENT',
                      `DTSTART:${dateStr}T${startTime}00`,
                      `DTEND:${dateStr}T${endTime}00`,
                      `SUMMARY:${(committeeName + ' — ' + ev.title).replace(/,/g, '\\,')}`,
                  ...
                  a.href = url;
                  a.download = `turonmun_${committeeAbbr || 'schedule'}.ics`;
                  a.click();
