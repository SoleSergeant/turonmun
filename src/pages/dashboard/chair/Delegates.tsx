...
export default function ChairDelegates() {
  ...
  const handleExport = () => {
    ...
    const rows = assignedDelegates.map(delegate => {
      ...
      const raw = positionPaper?.status || 'not_submitted';
      const statusText = raw === 'not_submitted' ? 'Not Submitted' : raw === 'draft' ? 'Draft' : 'Submitted';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {positionPapers?.filter(p => p.status && p.status !== 'draft').length || 0}
              </p>
            </div>
          </div>
        </div>
        <div className="glass-card p-4 border border-white/10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-white">
                {(assignedDelegates?.length || 0) - (positionPapers?.filter(p => p.status && p.status !== 'draft').length || 0)}
              </p>
            </div>
          </div>
        </div>
      ...
      {filteredDelegates.length === 0 ? (
        ...
        <div className="space-y-4">
          {filteredDelegates.map((delegate, index) => {
            ...
            const rawStatus = positionPaper?.status || 'not_submitted';
            // Normalise: anything past draft (submitted/reviewed/approved/needs_revision) = done
            const paperStatus =
              rawStatus === 'draft' ? 'draft'
              : rawStatus === 'not_submitted' ? 'not_submitted'
              : 'submitted';
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  {hasPaper && (
                    <button
                      onClick={() => setViewingPaper(positionPaper)}
                      className="ml-4 px-4 py-2 bg-gold-500/20 hover:bg-gold-500/30 text-gold-400 rounded-lg transition-colors text-sm font-medium"
                    >
                      View Paper
                    </button>
                  )}
        ...
        {viewingPaper && (
          ...
          >
            <motion.div
              ...
              className="bg-diplomatic-900 border border-white/20 rounded-2xl p-6 w-full max-w-3xl max-h-[85vh] flex flex-col"
              onClick={(e) => e.stopPropagation()}
            >
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-lg font-bold text-white">Position Paper</h3>
                  <p className="text-xs text-white/40 mt-0.5">
                    {assignedDelegates.find(d => d.id === viewingPaper.application_id)?.full_name || 'Delegate'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  {viewingPaper.file_url && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="px-3 py-1.5 bg-gold-400/20 text-gold-400 rounded-lg text-xs font-bold hover:bg-gold-400/30 transition-colors"
                    >
                      Open PDF
                    </a>
                  )}
              ...
              <div className="flex-1 overflow-y-auto">
                {viewingPaper.content ? (
                  ...
                  <p className="text-white/40 text-sm italic text-center py-8">
                    No text content — delegate submitted a PDF file.
                  </p>
                )}
              </div>
            </motion.div>
