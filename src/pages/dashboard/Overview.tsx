...
function Overview() {
  ...
  return (
    ...
    >
      ...
      {/* Checked-in confirmation banner (only shown after arrival is logged at the door) */}
      {(application as any)?.checked_in_at && (
        <motion.div
          variants={itemVariants}
          className="glass-card p-5 border border-emerald-400/40 bg-gradient-to-r from-emerald-500/10 to-transparent flex items-center gap-4"
        >
          <div className="w-12 h-12 rounded-xl bg-emerald-400/20 border border-emerald-400/40 flex items-center justify-center text-2xl">🟢</div>
          <div>
            <p className="text-emerald-300 font-bold text-sm uppercase tracking-widest mb-1">Checked in</p>
            <p className="text-white text-lg font-semibold">You're all set — enjoy the conference!</p>
        ...
        <motion.div variants={itemVariants} className="glass-card p-0 lg:col-span-5 flex flex-col h-full overflow-hidden group">
          ...
          <div className="mt-auto bg-white/5 border-t border-white/10 p-5">
            <div className="flex items-center justify-between">
              ...
              <Link to="/dashboard/application" className="text-[10px] text-gold-400 hover:text-gold-300 font-bold uppercase tracking-wider transition-all hover:translate-x-1 flex items-center gap-1">
                View All Details
