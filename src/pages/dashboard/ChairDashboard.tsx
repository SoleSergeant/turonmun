...
const chairSidebarItems: ChairSidebarItem[] = [
  ...
  { id: 'awards', label: 'Awards', icon: Award, path: isChairSubdomain ? '/awards' : '/chair-dashboard/awards' },
...
export default function ChairDashboardLayout() {
  ...
  const [chairName, setChairName] = useState('Committee Chair');
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Always scope to the chair's assigned committee_id — even for superadmins.
        // The chair dashboard is role-scoped; admins use the admin panel for global views.
        const committeeId: string | null = adminUser.committee_id ?? null;

        setChairName(adminUser.full_name || 'Committee Chair');

        if (!committeeId) {
          // No committee assigned yet — show empty dashboard
          setCommittees([]);
          setApplications([]);
          setPositionPapers([]);
          setLoading(false);
        ...
        const [{ data: committeesData }, { data: applicationsData }, { data: papersData }] = await Promise.all([
  ...
  // A paper counts as "submitted" once it leaves draft — including graded ones
  // (submitted / reviewed / approved / needs_revision).
  const submittedPapers = positionPapers.filter(p => p.status && p.status !== 'draft').length;
