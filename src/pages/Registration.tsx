...
const Registration = () => {
  ...
  // Autosave the form to localStorage so applicants can resume after a refresh.
  const { restoredAt, clearDraft } = useFormAutosave(
    'turonmun:draft:delegate', formData, setFormData, step, setStep,
  );
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          ielts_score: formData.hasIELTS === 'yes' && formData.ieltsScore
            ? parseFloat(formData.ieltsScore)
            : null,
          sat_score: formData.hasSAT === 'yes' && formData.satScore
            ? parseInt(formData.satScore, 10)
            : null,
          ...
          // Explicit status values — DB CHECK constraints reject NULL and the
          // column defaults aren't reliably set in production.
          status: 'pending',
...
Photo URL: ${photoUrl || 'N/A'}
      ...
      // Submission succeeded — wipe the saved draft so the next visit starts fresh.
      clearDraft();
