...
import { Shield, ChevronRight, ChevronLeft, CheckCircle, Send, AlertCircle, Clock, Save } from 'lucide-react';
...
export default function ChairApplication() {
  ...
  const { restoredAt, clearDraft } = useFormAutosave(
    'turonmun:draft:chair', formData, setFormData, step, setStep,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      const basePayload: Record<string, any> = {
        ...
        payment_amount: 0,
        // Explicit status values — the DB CHECK constraints reject NULL,
      ...
      // Wipe the saved draft so a fresh visit starts from scratch.
      clearDraft();
      setSubmitted(true);
