...
const DelegateManagement = () => {
  ...
  const openAssignModal = (delegateId: string, committeeId: string) => {
    ...
    // Hard payment gate — only paid delegates can be allocated to a committee
    if (delegate.payment_status !== 'paid') {
      toast({
        title: 'Payment required',
        description: `${delegate.full_name} has not paid yet. Mark payment as "paid" before assigning a committee.`,
        variant: 'destructive',
      });
      return;
    }
  ...
  const handleAssignCommittee = async () => {
    ...
    // Defensive payment gate
    const target = delegates.find(d => d.id === delegateId);
    if (target && target.payment_status !== 'paid') {
      toast({ title: 'Payment required', description: 'Only paid delegates can be assigned.', variant: 'destructive' });
      return;
    }
  ...
  const updatePaymentStatus = async (id: string, newStatus: string) => {
    try {
      const { error } = await supabase
        ...
        .eq('id', id);
      if (error) throw error;
      setDelegates(prev => prev.map(d => d.id === id ? { ...d, payment_status: newStatus as any } : d));
  ...
  const toggleContacted = async (id: string, current: boolean) => {
    ...
    try {
      const { error } = await supabase
        ...
        .eq('id', id);
      if (error) throw error;
      setDelegates(prev => prev.map(d => d.id === id ? { ...d, contacted: next } : d));
                    ...
                    <tr key={delegate.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        <div className="space-y-1.5">
                          {delegate.payment_amount != null && (
                            <div className="text-sm font-bold text-gray-800">
                              {delegate.payment_amount.toLocaleString()} UZS
                            </div>
                          )}
                          <select
                            value={delegate.payment_status}
                            onChange={(e) => updatePaymentStatus(delegate.id, e.target.value)}
                            className={`text-xs font-semibold rounded-full px-2 py-1 border-0 cursor-pointer focus:ring-2 focus:ring-offset-1 focus:ring-blue-400 ${getPaymentColor(delegate.payment_status)}`}
                          >
                            <option value="pending">pending</option>
                            <option value="paid">paid</option>
                            <option value="overdue">overdue</option>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex flex-col items-start gap-1.5">
                          <button
                            onClick={() => toggleContacted(delegate.id, delegate.contacted)}
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold transition-colors ${
                              delegate.contacted
                                ? 'bg-green-100 text-green-800 hover:bg-green-200'
                                : 'bg-gray-100 text-gray-500 hover:bg-gray-200'
                            }`}
                            title={delegate.contacted ? 'Click to mark as not contacted' : 'Click to mark as contacted'}
                          >
                            {delegate.contacted ? <Check className="h-3 w-3" /> : <Clock className="h-3 w-3" />}
                            {delegate.contacted ? 'Contacted' : 'Not contacted'}
                          </button>
                          {delegate.telegram_username && (
                            <a
                              href={`https://t.me/${delegate.telegram_username.replace('@', '')}`}
                              target="_blank"
                              rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 text-[11px] text-[#0088cc] hover:underline font-medium"
                            >
                              <MessageSquare className="h-3 w-3" /> {delegate.telegram_username}
                            </a>
                          )}
                        </div>
                      </td>
