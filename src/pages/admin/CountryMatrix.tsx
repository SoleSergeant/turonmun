...
const CountryMatrix = () => {
  ...
  const fetchAllData = async () => {
    ...
    try {
      ...
      const rosterCountriesList = (committeesData || []).flatMap((c: any) => c.countries || []).filter(Boolean);
  ...
  // Union of every committee's configured country roster + any already-assigned
  // country. Once rosters exist, the matrix only shows these — unused countries
  // from the global list are hidden.
  const rosterSet = new Set<string>();
  committees.forEach(c => ((c as any).countries || []).forEach((x: string) => {
    if (x) rosterSet.add(x.toLowerCase());
  }));
  matrix.forEach(m => {
    if (Object.values(m.committees).some(cell => cell.assigned)) {
      rosterSet.add(m.country.toLowerCase());
    }
  });
  const useRosterFilter = rosterSet.size > 0;

  const filteredMatrix = matrix.filter(item => {
    // Hide countries not used by any committee (only once rosters are set)
    if (useRosterFilter && !rosterSet.has(item.country.toLowerCase())) return false;
  ...
  // How many seats each committee has filled (for capacity-based hiding)
  const committeeFilled: Record<string, number> = {};
  committees.forEach(c => {
    committeeFilled[c.id] = matrix.reduce(
      (acc, m) => acc + (m.committees[c.id]?.assigned ? 1 : 0),
      0
    );
  });
  const committeeIsFull = (committeeId: string) => {
    const c = committees.find(x => x.id === committeeId);
    const total = (c as any)?.total_spots ?? 20;
    return (committeeFilled[committeeId] || 0) >= total;
  };
                        ...
                        return (
                          <td key={committee.id} className="px-4 py-4 text-center">
                            <div className="flex flex-col items-center gap-2">
                              ...
                              ) : committeeIsFull(committee.id) ? (
                                // Committee has no open seats — hide the assign control
                                <span className="text-xs text-gray-300 select-none">— full —</span>
                              ) : (
                                <div className="w-full">
                                  <select
                                    onChange={(e) => {
                                  ...
                                  >
                                    ...
                                    {delegates
                                      .filter(d => d.payment_status === 'paid' && !matrix.some(m =>
